...
echo
echo "Setting up SSL"
openssl req -new -x509 -nodes -out /etc/openldap/certs/dadcorp.pem -keyout /etc/openldap/certs/dadcorp.key -days 365 -subj '/C=US/L=Yes/O=Yes/OU=LA/CN=server.dadcorp.com' > /dev/null 2>&1
chown -R ldap:ldap /etc/openldap/certs
chmod 600 /etc/openldap/certs/dadcorp.key
...
dn: ou=People,dc=dadcorp,dc=com
objectClass: organizationalUnit
ou: People

dn: ou=Group,dc=dadcorp,dc=com
objectClass: organizationalUnit
ou: Group
EOF

echo "Adding base config"
ldapadd -x -w 1234 -D cn=ldapadm,dc=dadcorp,dc=com -f /tmp/base.ldif > /dev/null 2>&1
...
ldapadd -x -w 1234 -D cn=ldapadm,dc=dadcorp,dc=com -f $LDIF > /dev/null 2>&1
